import { BADGES, BadgeDefinition } from '../types/badges';
...
/**
 * Database adapter interface that BadgeService expects.
 * Implement this interface with your app's database layer.
 */
export interface BadgeDatabase {
...
/**
 * Event context passed to badge checks.
...
/**
 * Create a BadgeService instance with your database adapter.
 * 
 * @example
 * const badgeService = createBadgeService(myDatabaseAdapter);
 * const unlocked = await badgeService.checkUnlock('habit_complete', { streak: 7 });
 */
export function createBadgeService(db: BadgeDatabase) {
    return {
        /**
         ...
         */
        isRelevantEvent(badge: BadgeDefinition, eventType: BadgeEventType): boolean {
         ...
         */
        async evaluateRequirement(badge: BadgeDefinition, context: BadgeEventContext): Promise<boolean> {
            ...
            switch (type) {
                ...
                case 'time_of_day':
                    ...
                    if (this.checkTimeCondition(context.timestamp || Date.now(), meta)) {
            ...
            // Update progress for UI if tracked
...
/**
 * Type for the badge service instance.
 */
